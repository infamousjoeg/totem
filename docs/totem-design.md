...
- Root CA generated at `issuer init`, held offline or in KMS. Intermediate signs everything and rotates every 30 days with overlap; the bundle endpoint publishes current, next, and any intermediate still inside its retirement tail. Root rotation is a deliberate operator action.
- Three intermediates are live during a rotation, not two: the published successor that is trusted but not yet signing, the one signing now, and the outgoing one that has stopped signing but stays in the bundle until every leaf it minted has expired. "Current plus next" alone would break every unexpired leaf at the instant of handover, so the retirement tail must exceed the maximum SVID lifetime. Exactly one intermediate signs at any instant: the overlap is an overlap of publication and validity, never of signing, because two signers at once means a leaf's issuer cannot be derived from the clock and CRL scoping becomes guesswork.
...
- Policy is signed by admin devices, not by the issuer. Every change to enrollments, presence policy, grants, admin flags, and step-up approvals is a record signed with an admin device's presence-gated key; the issuer refuses to apply anything unsigned and stores the signed records in the same hash chain as issuance. The issuer host is a verifier and a cache of admin-signed state. A compromised host can mint under the CA (bounded by KMS) but cannot forge, alter, or insert a policy record: every one carries an admin device's signature it does not hold, and the chain makes an edit detectable.

What a compromised host CAN do is truncate. The chain proves its own consistency and nothing more, so removing records from the end and rewriting the recorded tip to match leaves a prefix that is internally valid, with no broken link and no gap in the ordinals. That reverts whatever those records did, and the case that matters is un-revoking: cutting back past a `revoke-admin` or a grant revocation restores access the human took away. Detecting it needs a value the attacker does not control. A witness the issuer host writes for itself is not that value: in this scenario the host holds the signing key and can produce any head it likes, freshly signed. What it cannot do is retract a value that has already left the box.

So the witness is the fleet. Every agent remembers the highest `(seq, hash)` the issuer has shown it, and on each contact requires the issuer to demonstrate that record is still in the chain it is serving now. An issuer truncated back past that point can no longer answer, because answering means producing records it deleted; claiming a different hash fails against the one the agent already holds. The agent refuses and alarms. This buys the property without a new box to own or a service to keep up, which matters for a laptop deployment, and the evidence is distributed before the compromise rather than defended during it. The cost is a protocol field, persistent agent-side state, and an endpoint that answers a consistency question. That is its own piece of work rather than a line in an existing one, and the deadline is the first real deployment: the guarantee is claimed to whoever runs this, so it holds before anyone does, not after.

The restore half is closed now and separately: a restore that would move the chain backwards, or onto a chain that is not the live one's own history, is refused rather than reasoned about. An operator who genuinely wants an older chain restores into a fresh directory, which is the safer procedure regardless.

Until the fleet check ships the guarantee above is "cannot change history", not "cannot roll it back", and the issuer states its head at open and after every restore so the log stream, which already leaves the box, carries the value a human can compare. That is a smaller thing than the check and it is not a substitute for it.
...
Rotation is per-reference and every reference must declare its shape. A credential a remote service accepts as soon as it changes is pull-rotated. A value that seals material at rest is not, because nothing here can re-seal what is already on disk: replacing it leaves the running process working and the next start broken, against files sealed under a value that is by then gone. Three secrets are of the second kind. The CA passphrase seals the intermediate private keys and the data key envelope-encrypts the store, and both of those brick at a restart, on a machine you still have, with the old value merely lost. The backup passphrase is worse and should be treated as such: it seals backups that outlive the issuer that wrote them, so it breaks at a restore, which is the one moment there is no old value to recover and no working system to recover it from. If one of these changes anyway, the issuer keeps serving the value its files are sealed under and says so on every rotation and every use, so the alarm is raised while both halves of a re-seal are still in memory. Adopting the new value is a deliberate command after the material has actually been re-sealed, never a timer.
